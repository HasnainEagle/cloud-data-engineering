...
SELECT 
	order_id,
	SUM(quantity * list_price * (1-discount)) Net_Revenuee
...
SELECT 
	category_id,
	ROUND(AVG(list_price),2) Avg_ListPrice
...
SELECT 
	first_name + ' ' + last_name Customer_FullName,
	SUM(quantity * list_price * (1-discount)) Total_Amount_Spent
FROM sales.customers cust
JOIN sales.orders ord
ON cust.customer_id = ord.customer_id
